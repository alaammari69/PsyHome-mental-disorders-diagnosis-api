...
def save_user_text(runtime: ToolRuntime[PatientContext], user_prompt: str)->None:
        ...
        print_debug(f"context : {runtime.context}")
...
def get_expected_symptom(runtime: ToolRuntime[PatientContext])-> Symptom | None:
    ...
    try:
        # retrive current context info from the db
        thread_info = PatientThreadDAO.get(thread_id=int(runtime.context.thread_id)).iloc[0]
        runtime.context.stage_of_diagnosis = StageOfDiagnosis[thread_info["stage_of_diagnosis"]]

        runtime.context.expected_symptom_id = int(thread_info["context_symptom_id"]) if thread_info["context_symptom_id"] is not None else None

        if runtime.context.expected_symptom_id is not None:
            expected_symptom_df = SymptomDAO.get(symptom_id=runtime.context.expected_symptom_id).iloc[0]

            runtime.context.expected_symptom = Symptom(
                symptom_id=int(expected_symptom_df["symptom_id"]),
                disorder_id=int(expected_symptom_df["disorder_id"]),
                symptom_name=expected_symptom_df["symptom_name"],
                symptom_description=expected_symptom_df["symptom_description"]
            )
...
def is_expected_symptom_confirmed(runtime: ToolRuntime[PatientContext], confirmation: str)->None:
    ...
    try:
        ...
        if expected_symptom is not None:
            ...
            runtime.context.expected_symptom_id = None
            runtime.context.expected_symptom = None

            PatientThreadDAO.update_context_symptom_id(thread_id=int(runtime.context.thread_id), context_symptom_id=None)
        else:
    ...
    try:
        ...
        if not all_related_symptoms:
            if runtime.context.stage_of_diagnosis == StageOfDiagnosis.START:  # in this case the symptom extraction stage is still in the beginning and we havn't extracted any symptom yes
                ...
                PatientThreadDAO.update_stage_of_diagnosis(thread_id=int(runtime.context.thread_id), stage_of_diagnosis=runtime.context.stage_of_diagnosis)
                return
        #***********************************
        ...
        if runtime.context.expected_symptom_id: # in case theres an expected symptom that's already graded
            ids_to_drop.append(runtime.context.expected_symptom_id)
...
@tool
def is_diagnosis_stage(runtime: ToolRuntime[PatientContext])->str|None:
    """
...
def commit_expected_symptom(runtime: ToolRuntime[PatientContext], expected_symptom_id: int)->Symptom|None:
    ...
    try:
        ...
        if expected_symptom_id != -1:
            ...
            print_debug(expected_symptom_id)

            runtime.context.expected_symptom_id = expected_symptom_id
            PatientThreadDAO.update_context_symptom_id(thread_id=int(runtime.context.thread_id), context_symptom_id=expected_symptom_id)
